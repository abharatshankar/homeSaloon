...
#define SLIDER_BUTTON_WIDTH         35.0f

#define DEFAULT_SLIDER_FRAME               CGRectMake(SLIDER_BUTTON_WIDTH/2, CGRectGetMidY(self.bounds), CGRectGetWidth(self.bounds) - SLIDER_BUTTON_WIDTH, 7)
...
@property (nonatomic, strong) UIImageView *minRangeImage;
@property (nonatomic, strong) UIImageView *maxRangeImage;
...
- (void)layoutSubviews
{
    ...
    if (self.numberOfSegments >= 2)
    {
        ...
        if ([self isRangeSlidersPlacedAtExtremePosition])
        {
            ...
            [self.startSliderButton setImage:[UIImage imageNamed:@"Load"] forState:UIControlStateNormal];
            self.startSliderButton.imageView.layer.masksToBounds = YES;
            self.startSliderButton.imageView.layer.cornerRadius = self.startSliderButton.imageView.frame.size.width/2;
            
            
            [self.endSliderButton setImage:[UIImage imageNamed:@"Load"] forState:UIControlStateNormal];
            self.endSliderButton.imageView.layer.masksToBounds = YES;
            self.endSliderButton.imageView.layer.cornerRadius = self.startSliderButton.imageView.frame.size.width/2;
            
            self.minRangeImage.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMidY(self.bounds) - SLIDER_BUTTON_WIDTH);
            self.maxRangeImage.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMidY(self.bounds) - SLIDER_BUTTON_WIDTH);
...
- (void)setDefaultValues
{
    ...
    self.rangeSliderBackgroundColor = [UIColor colorWithRed:87.0/255.0 green:96.0/255.0 blue:124.0/255.0 alpha:1.0];
    self.rangeSliderForegroundColor = [UIColor colorWithRed:87.0/255.0 green:96.0/255.0 blue:124.0/255.0 alpha:1.0];
    
    self.rangeDisplayLabelFont = [UIFont fontWithName:@"Poppins-Regular" size:12];
    self.rangeDisplayLabelColor = [UIColor whiteColor];
    
    self.segmentSelectedColor =  [UIColor colorWithRed:87.0/255.0 green:96.0/255.0 blue:124.0/255.0 alpha:1.0];;
    self.segmentUnSelectedColor = [UIColor colorWithRed:87.0/255.0 green:96.0/255.0 blue:124.0/255.0 alpha:1.0];
    

    
    self.minRangeImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 65, 40)];
    self.minRangeImage.center = CGPointMake(SLIDER_BUTTON_WIDTH / 2, CGRectGetMidY(self.bounds) - SLIDER_BUTTON_WIDTH-10);
    self.minRangeImage.image = [UIImage imageNamed:@"Union 2"];
    [self addSubview:self.minRangeImage];
    
    
    self.minRangeLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, -2, 60, 40)];
    //self.minRangeLabel.frame = self.minRangeImage.bounds;
    //self.minRangeLabel.layer.masksToBounds = YES;
    ...
    self.maxRangeLabel.adjustsFontSizeToFitWidth = YES;
    [self.minRangeImage addSubview:self.minRangeLabel];
    
    
    self.maxRangeImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 65, 40)];
    self.maxRangeImage.center = CGPointMake([self sliderMidPointForPoint:CGRectGetMaxX(self.bounds)], CGRectGetMidY(self.bounds) - SLIDER_BUTTON_WIDTH-20);
    self.maxRangeImage.image = [UIImage imageNamed:@"Union 2"];
    [self addSubview:self.maxRangeImage];
    
    
    self.maxRangeLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, -2, 60, 40)];
    //self.maxRangeLabel.frame = self.maxRangeImage.bounds;
    //self.maxRangeLabel.layer.masksToBounds = YES;
    ...
    self.maxRangeLabel.adjustsFontSizeToFitWidth = YES;
    [self.maxRangeImage addSubview:self.maxRangeLabel];
...
- (void)initSliderViews
{
    ...
    self.sliderBackgroundView.layer.cornerRadius = 4;
...
//    self.sliderForegroundView.layer.masksToBounds = YES;
//    [self.sliderForegroundView.layer insertSublayer:gradient atIndex:0];
//
    ...
    if ([self.startSliderButton isSelected])
    {
        if ([self shouldStartButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.minRangeImage.center = CGPointMake(CGRectGetMidX(self.startSliderButton.frame), CGRectGetMidY(self.startSliderButton.frame)-39);
    ...
    else if ([self.endSliderButton isSelected])
    {
        if ([self shouldEndButtonSlideForPoint:point])
        {
            [UIView animateWithDuration:0.1 animations:^{
                ...
                self.maxRangeImage.center = CGPointMake(CGRectGetMidX(self.endSliderButton.frame), CGRectGetMaxY(self.endSliderButton.frame)-56);
...
- (void)callScrollDelegate
{
    ...
    {
        minPercent = (CGRectGetMinX(self.startSliderButton.frame) / CGRectGetWidth(self.sliderBackgroundView.frame) * 100);
        maxPercent = (CGRectGetMinX(self.endSliderButton.frame) / CGRectGetWidth(self.sliderBackgroundView.frame) * 100);
...
self.minRangeLabel.text = [NSString stringWithFormat:@"%@ SAR",self.minRangeText];
self.maxRangeLabel.text =  [NSString stringWithFormat:@"%@ SAR",self.maxRangeText];
